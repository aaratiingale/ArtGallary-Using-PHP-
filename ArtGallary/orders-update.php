...
if(isset($_SESSION['cart'])) {
  ...
  foreach($_SESSION['cart'] as $Art_ID => $quantity) {

    $result = $mysqli->query("SELECT * FROM ART WHERE id = ".$Art_ID);
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        if($query){
          ...
          if($mysqli->query("UPDATE products SET qty = ".$newqty." WHERE id = ".$Art_ID)){
